                    ...
                    if(phoneCheck(uphone) == false){
                        ...
                        if(ucode.val()){
                            if($(".province option:selected").text()=='上课区域 省/市'||$(".city option:selected").text()=='上课区域 市/区'||$(".city option:selected").text()=='工作区域 市/区'||$(".city option:selected").text()=='工作区域 市/区'){
                                ...
                                uplace.parents('.infor-item').removeClass('infor-wrong');
                                return true;
                            }
                        }else{
                            $('.refer').removeClass('refer-on');
    ...
    if($('.dfth-listen').get(0)){
        ...
        btn1.on('click', function() {
            ...
            if (phoneCheck(uphone) == false) {
                ...
                $.ajax({
                    type:'GET',
                    cache:'false',
                    url:'http://192.168.1.227:8080/LandPsendCode',
        ...
        $('.refer').on('click',function(){
            if($('.refer').hasClass('refer-on')){
                ...
                var code = $('.ucode').val();
                var city1 = $(".province").find("option:selected").text();
                var city2 = $(".city").find("option:selected").text();
                var city3 = city1+city2;
                $.ajax({
                    type:'GET',
                    cache:'false',
                    url:'http://192.168.1.227:8080/LandApplyListen',
                    data:{'phone':uphone,'name':uname,'code':code,'area':city3},
                    dataType:'json',
                    success:function(data){
                        if(data.status===0){
                            $('.pop-fail').removeClass('hide');
                            $('.pop-fail').find('.p4').text(data.msg);
                        }else{
